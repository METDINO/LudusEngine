...
bool SubsystemStack::InitAll(const BootConfig& config) {
    ...
    for (std::size_t i = 0; i < m_entries.size(); i++) {
        ...
        if (entry.system->Init(config)) {
            ENGINE_LOG_INFO(Channels::kCore, "[{}/{}] {} started", i + 1, m_entries.size(), entry.name);
            ++m_startedCount;
            continue;
        }

        ENGINE_LOG_ERROR(Channels::kCore, "'{}' failed to start. shutting down the {} that did start", entry.name, m_startedCount);
